...
int solve(TinySolver *solver)
{
    ...
    for (int i = 0; i < solver->settings->max_iter; i++)
    {
        ...
        if (solver->settings->adaptive_rho) {
            ...
            __attribute__((unused)) tinytype pri_res_input = (solver->work->u - solver->work->znew).cwiseAbs().maxCoeff();
            __attribute__((unused)) tinytype pri_res_state = (solver->work->x - solver->work->vnew).cwiseAbs().maxCoeff();
            __attribute__((unused)) tinytype dua_res_input = solver->cache->rho * (solver->work->znew - z_prev).cwiseAbs().maxCoeff();
            __attribute__((unused)) tinytype dua_res_state = solver->cache->rho * (solver->work->vnew - v_prev).cwiseAbs().maxCoeff();
#else
            [[maybe_unused]] tinytype pri_res_input = (solver->work->u - solver->work->znew).cwiseAbs().maxCoeff();
            [[maybe_unused]] tinytype pri_res_state = (solver->work->x - solver->work->vnew).cwiseAbs().maxCoeff();
            [[maybe_unused]] tinytype dua_res_input = solver->cache->rho * (solver->work->znew - z_prev).cwiseAbs().maxCoeff();
            [[maybe_unused]] tinytype dua_res_state = solver->cache->rho * (solver->work->vnew - v_prev).cwiseAbs().maxCoeff();
#endif
